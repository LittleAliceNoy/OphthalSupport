...
import { useAdminRules } from './components/admin/hooks/useAdminRules';
...
export default function AdminPage({ config, onRefresh, isOffline, onEditingChange }: AdminPageProps) {
    ...
    const rules = useAdminRules({ config, onRefresh, showToast, setLoading });

    const hasUnsavedChanges = () => prices.hasUnsavedChanges() || operations.hasUnsavedChanges() || rules.hasUnsavedChanges();

    useEffect(() => {
        onEditingChange(hasUnsavedChanges());
    }, [
        onEditingChange,
        prices.state.editingCategory,
        prices.state.editingPriceId,
        prices.state.editPricesData,
        operations.state.editOpId,
        operations.state.editOpName,
        operations.state.editOpCategory,
        operations.state.editOpKeywords,
        operations.state.editOpRules,
        rules.actionsState.showAddActionModal,
        rules.actionsState.newActionId,
        rules.actionsState.newActionItem,
        rules.actionsState.editingActionId,
        rules.actionsState.editActionItem,
        rules.actionsState.editActionActive,
        rules.surgeonsState.showAddPrefModal,
        rules.surgeonsState.newPrefSurgeon,
        rules.surgeonsState.newPrefTool,
        rules.surgeonsState.newPrefValue,
        rules.surgeonsState.editingPrefId,
        rules.surgeonsState.isEditingSurgeonGroups,
        rules.surgeonsState.editSurgeonGroupsText,
        rules.surgeonsState.initialSurgeonGroupsText,
        rules.surgeonsState.isEditingSurgeonPrefs,
        rules.reusableToolsState.showAddReusableToolModal,
        rules.reusableToolsState.selectedExistingToolId,
    ]);
    ...
    return (
        <AdminAuthGate>
            ...
            >
                ...
                {adminTab === 'actions_rules' && <AdminRulesPage config={config} isOffline={isOffline} rulesHook={rules} />}

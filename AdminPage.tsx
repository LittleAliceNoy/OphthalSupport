...
const CATEGORY_ORDER = ['Lens Surgery', 'Retina', 'Glaucoma', 'Cornea', 'General'];
...
export default function AdminPage({ config, onRefresh }: AdminPageProps) {
    ...
    const [editToolCategory, setEditToolCategory] = useState<string>('General');
    ...
    const filteredPrices = useMemo(() => {
        ...
        return [...filtered].sort((a, b) => {
            ...
            const catA = toolA?.category || TOOL_CATEGORIES[a.tool_id] || 'General';
            const catB = toolB?.category || TOOL_CATEGORIES[b.tool_id] || 'General';
    ...
    const categorizedPrices = useMemo(() => {
        ...
        filteredPrices.forEach(p => {
            const tool = config.tools.find(t => t.id === p.tool_id);
            const cat = tool?.category || TOOL_CATEGORIES[p.tool_id] || 'General';
            const finalCat = CATEGORY_ORDER.includes(cat) ? cat : 'General';
    ...
    const handleEditPrice = (price: DBPrice) => {
        ...
        setEditToolCategory(tool?.category || TOOL_CATEGORIES[price.tool_id] || 'General');
    };

-- OphthalSupport admin RPC functions
...
begin
    ...
    values (
        p_price->>'tool_id',
        p_price->>'sub_key',
        (p_price->>'csmbs_price')::numeric,
        (p_price->>'sss_price')::numeric,
        (p_price->>'ucs_price')::numeric,
        p_price->>'display_name'
    );
end;
$$;
...
begin
    ...
    from jsonb_to_recordset(p_order_updates) as changes(
        id text,
        sort_order integer
    )
...
create or replace function public.admin_update_operation_with_rules(
    p_operation jsonb,
    p_delete_rule_ids text[],
    p_insert_rules jsonb,
    p_update_rules jsonb
)
returns void
language plpgsql
security definer
...
begin
    ...
    set name = p_operation->>'name',
        ...
        keywords = array(
            select jsonb_array_elements_text(p_operation->'keywords')
        )
    where id::text = p_operation->>'id';

    if coalesce(array_length(p_delete_rule_ids, 1), 0) > 0 then
        delete from operation_rules
        where id::text = any(p_delete_rule_ids);
    end if;
